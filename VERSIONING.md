...
gemchain guard ask-github
gemchain update ask-github <new-version> --dry-run
gemchain update ask-github <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-github.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/github/version.rb` by gemchain, not by hand.
...
| `lib/ask/github/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
